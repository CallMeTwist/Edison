...
import { BackButton }    from '@/components/ui/BackButton'
import { BodyModel3D }   from '@/features/physio/components/BodyModel3D'
import { HeartbeatLine } from '@/features/physio/components/HeartbeatLine'
import { BODY_DATA, PHYSIO_SERVICES, PHYSIO_STATS } from '@/features/physio/data'
import type { BodyRegionId } from '@/services/types'
...
export const PhysioPage: React.FC = () => {
  ...
  // Listen for region-selected CustomEvent from BodyModel3D
  useEffect(() => {
    const handler = () => setHeartbeatKey(k => k + 1)
    window.addEventListener('physio:region-selected', handler)
    return () => window.removeEventListener('physio:region-selected', handler)
  }, [])

  // GSAP stagger on treatment tags when sel changes
  useEffect(() => {
    if (!tagsContainerRef.current) return
    const noMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches
    if (noMotion) return
    const children = tagsContainerRef.current.children
    if (children.length === 0) return
    const tween = gsap.fromTo(
      children,
      { opacity: 0, y: 10 },
      { opacity: 1, y: 0, stagger: 0.055, duration: 0.35, ease: 'power2.out' }
    )
    return () => { tween.kill() }
  }, [sel])
  ...
  return (
    <div
      style={{
        ...
        animation:     'worldIn .65s ease forwards',
        background:    'radial-gradient(ellipse at 28% 55%,rgba(5,20,45,.97) 0%,rgba(0,2,10,.99) 100%)',
        overflow:      'hidden',   // ← nothing escapes: stats can never scroll behind model
      }}
    >
      {/* Decorative grid */}
      <div style={{
        position:        'absolute', inset: 0,
        backgroundImage: 'linear-gradient(rgba(79,195,247,.04) 1px,transparent 1px),linear-gradient(90deg,rgba(79,195,247,.04) 1px,transparent 1px)',
      ...
      <div style={{
        ...
        background: 'linear-gradient(90deg,transparent,rgba(79,195,247,.28),transparent)',
      ...
      {/* ── 3D Model (flexShrink: 0 → never compressed) ── */}
      <div style={{
        flex:       '0 0 260px',    // fixed height, never shrinks
        position:   'relative',
        zIndex:      5,
        margin:     '0 auto',
        width:      '100%',
      ...
      {/* ── Stats grid (flexShrink: 0 → ALWAYS VISIBLE below model) ── */}
      <div style={{
        display:               'grid',
        gridTemplateColumns:   'repeat(4,1fr)',
        gap:                    6,
        padding:               '10px 14px 0',
        flexShrink:             0,     // ← this is the key: stats can never be pushed away
        zIndex:                 5,
      }}>
        {PHYSIO_STATS.map(s => (
          <div
          ...
          >
            <div style={{ fontFamily: "'Syne',sans-serif", fontSize: 16, fontWeight: 800, color: ACC }}>
              {s.value}
            </div>
            <div style={{ fontSize: 8, color: 'rgba(255,255,255,.35)', fontFamily: "'Space Mono',monospace", lineHeight: 1.4, marginTop: 2 }}>
              {s.label}
            </div>
          </div>
        ))}
      ...
      {/* ── Hint text (flexShrink: 0) ── */}
      <div style={{ textAlign: 'center', padding: '6px 0 4px', flexShrink: 0, zIndex: 5 }}>
        <span style={{
      ...
      {/* ── Info panel (flex: 1 + overflowY: auto → scrolls inside its own box) ──
           This is the ONLY element that can grow or scroll.
           The stats above it can never move because they are flexShrink: 0.        ── */}
      <div style={{
        flex:       1,
        overflowY: 'auto',
        padding:   '12px 16px 32px',
        zIndex:     5,
        minHeight:  0,    // ← required for flex children to scroll correctly in Safari
      }}>

        {/* Default state — no region selected */}
        {!info ? (
          <div style={{ animation: 'fadeUp .5s ease forwards' }}>
            ...
            <p style={{
              ...
              Specialising in musculoskeletal rehabilitation, sports injuries,
              chronic pain management and movement re-education.
            </p>
            <p style={{
          ...
          /* Treatment Focus — selected state */
          <div key={sel} style={{ animation: 'fadeUp .35s ease forwards' }}>
            ...
            <div ref={tagsContainerRef} style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 18 }}>
              {info.tags.map(t => (
                <span
                  key={t}
                  style={{
            ...
            <div style={{ marginBottom: 14 }}>
              <HeartbeatLine key={heartbeatKey} active={heartbeatKey > 0} color={ACC} />
            </div>
            <button style={{
              padding:      '11px 24px',
              background:   'linear-gradient(135deg,rgba(79,195,247,.16),rgba(79,195,247,.04))',
              border:       '1px solid rgba(79,195,247,.38)',
              borderRadius:  100,
              color:         ACC,
              cursor:       'pointer',
              fontFamily:   "'Space Mono',monospace",
        ...
        {/* Services — always at the bottom of the scrollable area */}
      ...
      <div style={{ flex:1, display:'flex', flexDirection:'column', justifyContent:'center', padding:'88px 44px 40px', zIndex:5, gap:22, overflowY:'auto', animation:'fadeUp .8s .35s ease both' }}>
        ...
        {!info ? (
          ...
          <div key={sel} style={{ animation:'fadeUp .4s ease forwards' }}>
            ...
            <div ref={tagsContainerRef} style={{ display:'flex', flexWrap:'wrap', gap:7, marginBottom:24 }}>
              {info.tags.map(t => (
                <span key={t} style={{ padding:'5px 13px', background:'rgba(79,195,247,.08)', border:'1px solid rgba(79,195,247,.22)', borderRadius:100, color:ACC, fontSize:11 }}>{t}</span>
              ))}
            </div>
            <button onClick={() => navigateTo('contact')} style={{ padding:'12px 28px', background:'linear-gradient(135deg,rgba(79,195,247,.16),rgba(79,195,247,.04))', border:'1px solid rgba(79,195,247,.38)', borderRadius:100, color:ACC, cursor:'pointer', fontFamily:"'Space Mono',monospace", fontSize:10, letterSpacing:2, textTransform:'uppercase', transition:'all .3s' }}>
              Book Consultation →
            </button>
